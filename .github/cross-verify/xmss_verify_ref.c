/*
 * xmss_verify_ref.c - Verify rust-qrllib XMSS signature with reference implementation
 *
 * This verifier constructs a reference-compatible public key from rust-qrllib's
 * pk format and verifies the signature.
 *
 * rust-qrllib pk format: [root(32) | pub_seed(32)] = 64 bytes
 * Reference pk format: [OID(4) | root(32) | pub_seed(32)] = 68 bytes
 *
 * rust-qrllib sig format: [idx(4) | r(32) | WOTS_SIG(2144) | AUTH(h*32)] = 2500 bytes for h=10
...
int main() {
    uint8_t goqrllib_pk[64];     /* rust-qrllib pk: root || pub_seed */
    ...
    printf("  PK size (rust-qrllib): 64 bytes\n");
