 ...
 *
 * Counterpart to the rust-qrllib `xmss_sign` example: instead of
 * rust-qrllib producing a signature for the reference to verify, here
 * the reference produces a signature for rust-qrllib to verify (via
 ...
 * SK_SEED || SK_PRF, then 32 bytes for PUB_SEED) which matches the
 * 96-byte expanded-seed convention rust-qrllib's
 * `qrllib::xmss::rfc8391::new_keypair` uses on the Rust side. The link
 * command for this file therefore omits the upstream `randombytes.c`
 * to avoid duplicate-symbol errors; `xmss_verify_ref.c` still includes
 * it.
 *
 ...
 * expanded-seed semantics qrllib::xmss::rfc8391::new_keypair uses. */
...
int main(void) {
    /* The same deterministic 96-byte expanded seed that the Rust
     * reverse-direction verifier will pass into
     * `qrllib::xmss::rfc8391::new_keypair`. */
    ...
    memcpy(msg, "XMSS reference -> rust-qrllib bidirectional verification", 56);
    size_t msglen = 56;
    ...
    /* Write the artefacts the rust-qrllib verifier needs. The pk is
     * written WITHOUT the OID prefix (just root || pub_seed) for the
     * internal pk-bytes-match check; with the OID prefix for the
     * `qrllib::xmss::rfc8391::verify` call site. */
    FILE *f;
